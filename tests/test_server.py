...
from coreason_economist.server import app, get_db
from coreason_economist.database import BudgetAccount
...
def test_authorize_budget_success(client, mock_session):
    ...
    response = client.post(
        "/budget/authorize",
        json={"project_id": "p1", "estimated_cost": 0.5}
    )
...
def test_authorize_budget_insufficient_funds(client, mock_session):
    mock_result = MagicMock()
...
def test_authorize_budget_auto_provision(client, mock_session):
    ...
    response = client.post(
        "/budget/authorize",
        json={"project_id": "new_proj", "estimated_cost": 1.0}
    )
...
def test_commit_budget(client, mock_session):
    ...
    response = client.post(
        "/budget/commit",
        json={"project_id": "p1", "estimated_cost": 0.5, "actual_cost": 0.4}
    )
...
def test_voc_analyze(client):
    response = client.post(
        "/voc/analyze",
        json={"task_complexity": 0.5, "current_uncertainty": 0.2}
    )
    assert response.status_code == 200
    data = response.json()
