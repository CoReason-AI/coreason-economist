...
def test_voc_boundary_conditions(client: TestClient) -> None:
    # Threshold is: should_execute = not (task_complexity > 0.9 and current_uncertainty > 0.9)

    # Case 1: Both 0.9 (Should execute)
    r1 = client.post("/voc/analyze", json={"task_complexity": 0.9, "current_uncertainty": 0.9})
    assert r1.status_code == 200
    assert r1.json()["should_execute"] is True

    # Case 2: One > 0.9 (Should execute)
    r2 = client.post("/voc/analyze", json={"task_complexity": 0.95, "current_uncertainty": 0.9})
    assert r2.status_code == 200
    assert r2.json()["should_execute"] is True

    # Case 3: Both > 0.9 (Should NOT execute)
    r3 = client.post("/voc/analyze", json={"task_complexity": 0.91, "current_uncertainty": 0.91})
    assert r3.status_code == 200
    assert r3.json()["should_execute"] is False


def test_large_numbers(client: TestClient, mock_session: AsyncMock) -> None:
    # Handling large financial numbers
    mock_account = BudgetAccount(project_id="p1", balance=Decimal("1000000.0"))
    mock_session.execute.return_value.scalar_one_or_none.return_value = mock_account

    response = client.post("/budget/authorize", json={"project_id": "p1", "estimated_cost": 999999.0})
    assert response.status_code == 200
    assert mock_account.balance == Decimal("1.0")


def test_penny_pincher_workflow(client: TestClient, mock_session: AsyncMock) -> None:
    """
    Workflow:
    - Start 1.0
    - Spend 0.5 (Bal 0.5)
    - Spend 0.5 (Bal 0.0)
    - Try Spend 0.1 (Fail)
    - Commit (Refund 0.2) -> Bal 0.2
    - Spend 0.1 (Success) -> Bal 0.1
    """
    mock_account = BudgetAccount(project_id="pp", balance=Decimal("1.0"))
    mock_session.execute.return_value.scalar_one_or_none.return_value = mock_account

    # 1. Spend 0.5
    client.post("/budget/authorize", json={"project_id": "pp", "estimated_cost": 0.5})
    assert mock_account.balance == Decimal("0.5")
    ...
    # 3. Try Spend 0.1
    r_fail = client.post("/budget/authorize", json={"project_id": "pp", "estimated_cost": 0.1})
    assert r_fail.status_code == 402

    # 4. Commit previous with refund 0.2 (Est 0.5, Actual 0.3)
    client.post("/budget/commit", json={"project_id": "pp", "estimated_cost": 0.5, "actual_cost": 0.3})
    assert mock_account.balance == Decimal("0.2")

    # 5. Spend 0.1
    r_ok = client.post("/budget/authorize", json={"project_id": "pp", "estimated_cost": 0.1})
    assert r_ok.status_code == 200
    assert mock_account.balance == Decimal("0.1")
